...
from .models import Book,Review2
...
def home(request):
    return render(request,'home.html', {'name':'jy'})
...
def bookdetail(request, book_id):
    ...
    reviews = Review2.objects.filter(book=book)
...
def updatebookreview(request, review_id) :
    review = get_object_or_404(Review2, pk=review_id, user=request.user)
    if request.method == 'GET':
